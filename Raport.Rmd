...
•	__Net Conversion__: A number of user-ids to remain enrolled past the 14-day boundary (and thus make at least one payment) divided by the number of unique cookies to click the "Start free trial" button. (__Payments__/ __Number of Clicks__). The practical significance level d_min = 0.0075. I chose this metric because the change in this metric lets me to determine if the experiment had an effect on the number of uses who clicked and became paying customers after the free trial ends. To reject the null hypothesis, this metric has to stay at constant level of, while __Gross Conversion__ would show practically significant decrease.
...
Bonferroni Correction is used to correct for false positives, and it comes with the side-effect of decreasing power/increasing false negatives. It doesn't assume independence, is very conservative, it guarantees the alpha at least as small as specified. I didn't use it, since the metrics are correlated, for the method is too conservative.
...
The parameters to calculate the sample size for **Gross Conversion**:
...
cat(paste('Required number of pagevews: ', NC_size))

```

##### 5.2 Duration vs. Exposure

A test would be considered high risk if it causes harm to its users, or decline in enrollment, or user satisfaction when the screen provides some time commitment expectation.  I think, it will not impact existing paying customers, and it is safe to run the test on 100% of user traffic.
...
__Gross Conversion__ (Enrollments/ Clicks):
```{r, GS}
...
CI does contain (-)dmin, it is not practically significant.
